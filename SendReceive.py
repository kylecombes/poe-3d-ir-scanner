...
class PointCollection:
    ...
    # Add points to the scatter plot and the list
...
NUM_POINTS_TO_KEEP = 1100 # Number of points to keep on the scatter plot
MAX_DIST = 30  # Maximum distance (in), to remove outliers

# Initialize stuff
cxn = Serial('/dev/tty.usbmodem14131', baudrate=9600)
...
while True:
    (dist, theta, phi) = read_serial() # data coming in
    if dist and dist < MAX_DIST:
        # Scan and print cartesian points
        (x, y, z) = spherical_to_cartesian(dist, theta, phi) # converts to cartesian coordinates
        points.add_point(Point(x, y, z)) # add points to point collection
        print('Radius: {0:0.3f}\tAngle: {1}\tx:{2:0.3f}\ty:{3:0.3f}\tz:{3:0.3f}'.format(dist, theta, phi, x, y, z))
        # Plot our scan
        pyplot.cla()  # Clear figure
        pyplot.scatter(points.get_x_values(), c= points.get_y_values(), points.get_z_values(), vmin = 15, vmax = 30, s = 50)
